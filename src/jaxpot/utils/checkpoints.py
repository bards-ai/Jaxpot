...
def configure_best_checkpoint_selection(
    cfg: DictConfig, evaluator_eval_every_by_name: dict[str, int]
) -> BestCheckpointSelection:
    best_checkpoint_metric = cfg.get("best_checkpoint_metric")
    best_checkpoint_mode = str(cfg.get("best_checkpoint_mode", "max")).lower()
    best_checkpoint_enabled = False
    if best_checkpoint_metric:
        if best_checkpoint_mode not in {"max", "min"}:
            raise ValueError(
                f"Invalid best_checkpoint_mode='{best_checkpoint_mode}', expected one of ['max', 'min']"
            )
        best_checkpoint_metric = str(best_checkpoint_metric)
        best_checkpoint_metric_prefix = best_checkpoint_metric.split("/", 1)[0]
        metric_eval_every = evaluator_eval_every_by_name.get(best_checkpoint_metric_prefix)
        if metric_eval_every is None:
            raise ValueError(
                "No evaluator found with name "
                f"'{best_checkpoint_metric_prefix}' for metric '{best_checkpoint_metric}'. Can't save best checkpoints."
            )
        save_every = int(cfg.save_every)
        if save_every <= 0:
            logger.warning(
                "Disabling best checkpoint selection: save_every <= 0, regular checkpoints are off."
            )
        else:
            if metric_eval_every > 0 and save_every % metric_eval_every != 0:
                raise ValueError(
                    "best_checkpoint metric cadence may miss save iterations: "
                    f"save_every={save_every}, evaluator='{best_checkpoint_metric_prefix}', "
                    f"eval_every={metric_eval_every}."
                )
            best_checkpoint_enabled = True
    return BestCheckpointSelection(
        enabled=best_checkpoint_enabled,
...
class BestCheckpointManager:
    ...
    def maybe_save(
        ...
        if not self._is_better_than_worst(score):
            ...
            return None
        logger.info(f"Saving best checkpoint: iter={iteration}, score={score:.4f}")
