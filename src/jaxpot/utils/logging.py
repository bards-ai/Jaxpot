...
def _build_training_column(metrics: dict, log_payload: dict) -> Table:
    ...
    vl = float(metrics.get("value_loss", 0))
    pl = float(metrics.get("policy_loss", 0))
    ent = float(metrics.get("entropy_loss", 0))
    kl = float(metrics.get("kl_loss", 0))
    gn = float(metrics.get("grad_norm_clipped", metrics.get("grad_norm", 0)))

    t.add_row(Text("VL", style=_loss_color(vl)), Text(_fmt(vl), style=_loss_color(vl)))
    t.add_row(Text("PL", style=_loss_color(pl)), Text(_fmt(pl), style=_loss_color(pl)))
    t.add_row(Text("Ent", style="cyan"), Text(_fmt(ent), style="cyan"))
    t.add_row(Text("KL", style="magenta"), Text(_fmt(kl), style="magenta"))
    cf = float(metrics.get("clipfrac", 0))
    t.add_row(Text("Grad", style="dim"), Text(_fmt(gn), style="dim"))
    t.add_row(Text("Clip", style="dim"), Text(_fmt(cf), style="dim"))
